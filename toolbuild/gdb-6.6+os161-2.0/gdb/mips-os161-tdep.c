/* Target-dependent code for OS/161 on MIPS, for GDB,
   the GNU Debugger. Copied from mips-mdebug-tdep.c.
...
mips_os161_frame_cache (struct frame_info *next_frame, void **this_cache)
{
...
static void
mips_os161_frame_this_id (struct frame_info *next_frame, void **this_cache,
			   struct frame_id *this_id)
{
  struct mips_frame_cache *info = mips_os161_frame_cache (next_frame,
							   this_cache);
  (*this_id) = frame_id_build (info->base, frame_func_unwind (next_frame));
}

static void
mips_os161_frame_prev_register (struct frame_info *next_frame,
				 void **this_cache,
...
static const struct frame_unwind mips_os161_frame_unwind =
{
  NORMAL_FRAME,
  mips_os161_frame_this_id,
  mips_os161_frame_prev_register
};

static const struct frame_unwind *
mips_os161_frame_sniffer (struct frame_info *next_frame)
{
  ...
  kernel_trap = PROC_REG_MASK (proc_desc) & 1;
  if (kernel_trap)
    return &mips_os161_frame_unwind;
...
static CORE_ADDR
mips_os161_frame_base_address (struct frame_info *next_frame,
				void **this_cache)
{
  struct mips_frame_cache *info = mips_os161_frame_cache (next_frame,
							   this_cache);
...
static const struct frame_base mips_os161_frame_base = {
  &mips_os161_frame_unwind,
  mips_os161_frame_base_address,
  mips_os161_frame_base_address,
  mips_os161_frame_base_address
};

static const struct frame_base *
mips_os161_frame_base_sniffer (struct frame_info *next_frame)
{
  if (mips_os161_frame_sniffer (next_frame) != NULL)
    return &mips_os161_frame_base;
  else
    return NULL;
}

void
mips_os161_append_sniffers (struct gdbarch *gdbarch)
{
  frame_unwind_append_sniffer (gdbarch, mips_os161_frame_sniffer);
  frame_base_append_sniffer (gdbarch, mips_os161_frame_base_sniffer);
}
...
#define OS161_MIPS_JB_PC		1
#define OS161_MIPS_JB_ELEMENT_SIZE	mips_isa_regsize (current_gdbarch)
#define OS161_MIPS_JB_OFFSET		(OS161_MIPS_JB_PC * \
					 OS161_MIPS_JB_ELEMENT_SIZE)

static int
mips_os161_get_longjmp_target (CORE_ADDR *pc)
{
  CORE_ADDR jb_addr;
  char *buf;

  buf = alloca (OS161_MIPS_JB_ELEMENT_SIZE);

  jb_addr = read_register (MIPS_A0_REGNUM);

  if (target_read_memory (jb_addr + OS161_MIPS_JB_OFFSET, buf,
  			  OS161_MIPS_JB_ELEMENT_SIZE))
    return 0;

  *pc = extract_unsigned_integer (buf, OS161_MIPS_JB_ELEMENT_SIZE);

  return 1;
}

static void
mips_os161_init_abi (struct gdbarch_info info,
                   struct gdbarch *gdbarch)
{
  //set_gdbarch_regset_from_core_section
  //  (gdbarch, mips_os161_regset_from_core_section);

  set_gdbarch_get_longjmp_target (gdbarch, mips_os161_get_longjmp_target);
...
extern void _initialize_mips_os161_tdep (void);
void
_initialize_mips_os161_tdep (void)
{
  mips_pdr_data = register_objfile_data ();
  gdbarch_register_osabi (bfd_arch_mips, 0, GDB_OSABI_OS161,
			  mips_os161_init_abi);
}
